...
export default function MenuBebidas () {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const licores = allProducts.filter(
          (p: Product) =>
            p.type === 'bebida' &&
            (LICOR_CATEGORIES as readonly string[]).includes(p.category)
        )
        setProducts(licores)
      } catch (error) {
  ...
  const sorted = useMemo(
    () => sortMenuByIds(products, MENU_ORDER_LICORES_BOTELLA),
    [products]
  )
