...
import { MENU_ORDER_CAFETERIA, sortMenuByIds } from '@/lib/menu-display-order'
...
export default function MenuCafes() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const cafeProducts = allProducts.filter((p: Product) => 
          p.type === 'cafeteria' && 
          (p.category === 'cafe-caliente' || p.category === 'cafe-frio')
        )
  ...
  const sortedProducts = useMemo(
    () => sortMenuByIds(products, MENU_ORDER_CAFETERIA),
    [products]
  )
