...
import { MENU_ORDER_CERVEZA, sortMenuByIds } from '@/lib/menu-display-order'
...
export default function MenuCervezas() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const cervezaProducts = allProducts.filter((p: Product) => 
          p.type === 'bebida' && p.category === 'cerveza'
        )
        setProducts(cervezaProducts)
      } catch (error) {
  ...
  const sortedProducts = useMemo(
    () => sortMenuByIds(products, MENU_ORDER_CERVEZA),
    [products]
  )
